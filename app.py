...
with col1:
    ...
    a = st.number_input("Cận dưới a:", value=0.0, min_value=-1e12)
    b = st.number_input("Cận trên b:", value=1.0, min_value=-1e12)
...
try:
    ...
    def wrapper(x_arr):
        ...
        mask = ~np.isfinite(y)
        for i, xi in enumerate(x_arr[mask]):
            try:
                y_idx = float(sp.limit(f_expr, x, xi))
                y[np.where(mask)[0][i]] = y_idx
            except Exception:
                y[np.where(mask)[0][i]] = np.nan
        return y
    return wrapper
