...
with col2:
    ...
    if mode == "Nhập số khoảng n":
        n_input = st.number_input("Số khoảng chia n:", min_value=2, value=10, step=1)
        epsilon = None
    else:
...
try:
    # Hỗ trợ exp và hằng số e
    f_expr = sp.sympify(expr_str, locals={'e': sp.E, 'sp': sp})
...
if method in ["Hình thang", "Cả hai"]:
    I_trap, n_used_trap = compute_with_tolerance(f_lambda, a, b, trapezoidal_rule, epsilon, n_input)
...
if method in ["Hình thang", "Cả hai"]:
    ...
    text = f"Sai số thực nghiệm: {err_trap:.6f}" if err_trap is not None else ""
...
if method in ["Simpson", "Cả hai"]:
    ...
    text = f"Sai số thực nghiệm: {err_simp:.6f}" if err_simp is not None else ""
